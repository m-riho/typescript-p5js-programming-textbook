...
\section{JSDoc形式のコメントで関数の説明を書く}
ここでは、関数の定義の直前に書くコメントの書き方であるJSDoc形式のコメントを紹介します。
コメントの書き方には色々な流儀がありますが、
JSDoc形式のコメントはJavaScriptやTypeScriptの関数の説明を書くときに広く使われている書き方です。
この形式でコメントを書くと、関数の使い方や引数や戻り値の意味を整理して書けます。

この節の最後に掲載するリスト\ref{lst:chapter08-comment-sample-draw-eye}からリスト\ref{lst:chapter08-comment-sample-sketch-draw}のように、関数の定義の直前にコメントを書くのがJSDoc形式の書き方です。関数の目的、仮引数、戻り値の意味を整理して書くことができます。

JSDoc形式のコメントは、\texttt{/**}で始まり、\texttt{*/}で終わります。最初に関数の目的を書き、その下に\texttt{@param}で仮引数を、\texttt{@returns}で戻り値を説明します。リスト\ref{lst:chapter08-comment-sample-mouth-hit}の\texttt{isMouseOverMouth}では、口の範囲内なら\texttt{true}、範囲外なら\texttt{false}を返すことを\texttt{@returns}に書いています。

戻り値が\texttt{void}の関数では、呼び出し側へ値を返さないため、通常は\texttt{@returns}を省略できます。

このサンプルでは、マウスカーソルが口を囲む長方形の範囲内にあるかを\texttt{isMouseOverMouth}で調べます。戻り値が\texttt{true}のときだけ、カーソル位置へ\texttt{I'm Kyubey.}と表示します。関数の戻り値を\texttt{if}の条件に使う例にもなっています。

VS Codeを使っている場合、関数の定義の上で\texttt{/**}と入力してEnterを押すと、自動的にJSDoc形式のコメントのひな形が挿入されます。
また、プログラムを入力中に関数名を入力すると、関数の定義の上に書いたJSDoc形式のコメントが表示されます。図\ref{fig:chapter08-jsdoc}は、そのコメントが表示された例です。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/chapter08/vscode-popup.png}
    \caption{VS Codeで関数名を入力したときに表示されるJSDoc形式のコメント}
    \label{fig:chapter08-jsdoc}
\end{figure}

JSDocはコメントの書き方の名前であると同時に、コメントからHTML形式のAPIリファレンスを生成するツールの名前でもあります。TypeScriptでは、同じ目的でTypeDocも利用できます。

\clearpage
最後に、この節で説明したJSDoc形式のコメントを使ったサンプルプログラム全体を示します。1つのプログラムを関数単位に分けて掲載していますが、すべて\texttt{comment-sample.ts}に書かれた一続きのプログラムです。

\begin{listing}[htbp]
\inputminted[firstline=1,lastline=40]{ts}{listings/chapter08/comment-sample.ts}
\caption{JSDoc形式のコメントを付けた\texttt{drawEye}}
\label{lst:chapter08-comment-sample-draw-eye}
\end{listing}

\begin{listing}[htbp]
\inputminted[firstline=41,lastline=88]{ts}{listings/chapter08/comment-sample.ts}
\caption{JSDoc形式のコメントを付けた\texttt{drawMouth}}
\label{lst:chapter08-comment-sample-draw-mouth}
\end{listing}

\begin{listing}[htbp]
\inputminted[firstline=89,lastline=126]{ts}{listings/chapter08/comment-sample.ts}
\caption{JSDoc形式のコメントを付けた\texttt{drawMark}}
\label{lst:chapter08-comment-sample-draw-mark}
\end{listing}

\begin{listing}[htbp]
\inputminted[firstline=127,lastline=154]{ts}{listings/chapter08/comment-sample.ts}
\caption{顔全体を描く\texttt{drawKyubeyFace}（1）}
\label{lst:chapter08-comment-sample-draw-face-start}
\end{listing}

\begin{listing}[htbp]
\inputminted[firstline=155,lastline=179]{ts}{listings/chapter08/comment-sample.ts}
\caption{顔全体を描く\texttt{drawKyubeyFace}（2）}
\label{lst:chapter08-comment-sample-draw-face-middle}
\end{listing}

\begin{listing}[htbp]
\inputminted[firstline=180,lastline=205]{ts}{listings/chapter08/comment-sample.ts}
\caption{顔全体を描く\texttt{drawKyubeyFace}（3）}
\label{lst:chapter08-comment-sample-draw-face-end}
\end{listing}

\begin{listing}[htbp]
\inputminted[firstline=206,lastline=237]{ts}{listings/chapter08/comment-sample.ts}
\caption{口の範囲内にあるかを調べる\texttt{isMouseOverMouth}}
\label{lst:chapter08-comment-sample-mouth-hit}
\end{listing}

\begin{listing}[htbp]
\inputminted[firstline=238,lastline=264]{ts}{listings/chapter08/comment-sample.ts}
\caption{顔の位置と大きさを用意する\texttt{sketch}}
\label{lst:chapter08-comment-sample-sketch-setup}
\end{listing}

\begin{listing}[htbp]
\inputminted[firstline=265,lastline=307]{ts}{listings/chapter08/comment-sample.ts}
\caption{口の位置を調べて文字を表示する\texttt{draw}}
\label{lst:chapter08-comment-sample-sketch-draw}
\end{listing}
\clearpage

リスト\ref{lst:chapter08-comment-sample-draw-face-start}からリスト\ref{lst:chapter08-comment-sample-draw-face-end}の\texttt{drawKyubeyFace}は、顔の部品を描く関数を順番に呼び出して、顔全体を組み立てています。図\ref{fig:chapter08-draw-kyubey-face-calls}に、関数の呼び出し関係を示します。
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[function, fill=blue!8] (face) at (0, 0)
            {\texttt{drawKyubeyFace}\\顔全体を組み立てる};

        \node[part, fill=yellow!12] (mark) at (-4.5, -2.4)
            {\texttt{drawMark}\\額の模様を描く（1回）};
        \node[part, fill=green!10] (eye) at (0, -2.4)
            {\texttt{drawEye}\\左右の目を描く（2回）};
        \node[part, fill=red!8] (mouth) at (4.5, -2.4)
            {\texttt{drawMouth}\\口を描く（1回）};

        \draw[call] (face.south west) -- (mark.north);
        \draw[call] (face.south) -- (eye.north);
        \draw[call] (face.south east) -- (mouth.north);
    \end{tikzpicture}
    \caption{\texttt{drawKyubeyFace}から呼び出す関数}
    \label{fig:chapter08-draw-kyubey-face-calls}
\end{figure}

\texttt{drawKyubeyFace}を読むと、顔が「額の模様、左右の目、口」という部品からできていることが分かります。目や口をどのような図形で描くかという細かな処理は、それぞれ\texttt{drawEye}や\texttt{drawMouth}の中にまとめられています。そのため、顔全体の構成を確認するときに、すべての描画命令を一度に追う必要がありません。

このように、意味のある処理のまとまりに名前を付けて関数にすると、プログラムの役割や構造を読み取りやすくなります。また、左右の目のように同じ形を描く場合は、同じ\texttt{drawEye}を実引数だけ変えて再利用できます。
\clearpage
